...
class WinderKinematics:
    ...
    def set_position(self, newpos, homing_axes):
        logging.info("DEBUG WINDER set_position: newpos=%s homing_axes=%s current_limits=%r" % (newpos, homing_axes, self.limits[1]))
        self.rail.set_position(newpos)
        if 'y' in homing_axes:
            # Update limits when homing
            self.limits[1] = self.rail.get_range()
            logging.info("DEBUG WINDER set_position: Updated limits after homing: %r" % (self.limits[1],))
        # After any move, ensure limits are still valid if we were previously homed
        elif self.limits[1][0] <= self.limits[1][1]:
            # We're already homed, keep the limits valid
            # Just verify the position is still within range
            logging.info("DEBUG WINDER set_position: Limits still valid after move, keeping them")
        # else: limits are invalid (not homed yet) - this is expected during initialization
    
    def clear_homing_state(self, clear_axes):
        if 'y' in clear_axes:
            logging.warning("DEBUG WINDER clear_homing_state: Clearing Y-axis limits! clear_axes=%s" % clear_axes)
    ...
    def _home_traverse(self, homing_state):
        ...
        # Get current position to see where we're starting from
        try:
        ...
        # Use very large safety margin to ensure we can reach endstop from anywhere
        # This accounts for incorrect rotation_distance calibration
        safety_margin = 150.0  # Increased margin to ensure we can reach switch
        if hi.positive_dir:
            forcepos[1] = position_min - safety_margin
        else:
            forcepos[1] = position_max + safety_margin
        
        logging.info("DEBUG WINDER HOMING: position_min=%.2f position_max=%.2f position_endstop=%.2f positive_dir=%s current_pos=%s forcepos=%.2f" 
                    % (position_min, position_max, hi.position_endstop, hi.positive_dir, current_pos, forcepos[1]))
        
        # Perform homing
        homing_state.home_rails([self.rail], forcepos, homepos)
        
        logging.info("DEBUG WINDER HOMING: homing completed")
    ...
    def check_move(self, move):
        ...
        # Check Y-axis limits
        if move.axes_d[1]:
            logging.info("DEBUG WINDER check_move: end_pos[1]=%.3f limits[1]=%r" % (end_pos[1], self.limits[1]))
